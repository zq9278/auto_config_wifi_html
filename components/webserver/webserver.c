...
extern const uint8_t root_start[] asm("_binary_index_html_start"); // HTML起始指针

extern const uint8_t root_end[] asm("_binary_index_html_end"); // HTML结束指针

static const char *TAG = "WEB_SERVER"; // 日志标签
static esp_err_t get_wifi_list(httpd_req_t *req);
...
static esp_err_t http_SendText_html(httpd_req_t *req)
{
    ...
    httpd_resp_send(req, (const char *)root_start, root_len);
...
static esp_err_t wifiscan(httpd_req_t *req)
{
    ...
    ESP_LOGI(TAG, "dsssssssssssssssssssssssssssssssssssssssssssssssss");
    ...
    wifi_ap_record_t ap_records[6];
    ap_count = ap_count > 6 ? 6 : ap_count;
    ...
    strcat(wifi_list, "</ul>");
    //printf("%s", wifi_list);
    httpd_resp_set_type(req, "text/html");// 设置Content-Type为text/html
    httpd_resp_set_hdr(req, "Content-Encoding", "utf-8"); // 设置编码为UTF-8
    httpd_resp_send_chunk(req, (const char *)wifi_list, strlen(wifi_list));
    httpd_resp_send_chunk(req, NULL, 0);

    //  httpd_resp_set_type(req, "text/html");
    //  httpd_resp_send_chunk(req, "<li onclick=\"selectWifi(this)\">7685555555555</li>", sizeof("<li onclick=\"selectWifi(this)\">7685555555555</li>"));
    //  httpd_resp_send_chunk(req, NULL, 0);
    return ESP_OK;
}
static esp_err_t get_wifi_list(httpd_req_t *req)
...
void web_server_start(void)
{
    ...
    httpd_uri_t get_wifi = {
        .uri = "/get_wifi_list",  // 匹配所有URI
        .method = HTTP_GET,       // 请求方法为GET
        .handler = get_wifi_list, // 处理函数为HTTP_FirstGet_handler
        .user_ctx = NULL,         // 用户上下文为NULL
    };
    httpd_register_uri_handler(server, &get_wifi); // 在服务器上注册URI处理器
